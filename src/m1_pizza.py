...
def run_test_polygon():
    ...
    #Test 3
    title=('Polygon Test 3: 4 segments with thick black lines on a lime green circle')
    window=rg.RoseWindow(550,400, title)
    circle=rg.Circle(rg.Point(350,200),150)
    circle.outline_thickness=3
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(pts)):
        if k<len(pts)-1:
            l1=rg.Line(pts[k],pts[k+1])
            l1.color=color
            l1.thickness=thickness
            l1.attach_to(window)
        else:
            l1=rg.Line(pts[k],pts[0])
            l1.color = color
            l1.thickness = thickness
            l1.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  3000 white lines on green circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    stpt = 0
    for k in range(len(pts)):
        endpt=stpt+hops_to_next_point
        ind=endpt%len(pts)
        l1=rg.Line(pts[stpt],pts[ind])
        l1.color=color
        l1.thickness=thickness
        l1.arrow='last'
        l1.attach_to(window)
